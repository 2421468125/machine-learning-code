...
class DecisionTree:
    ...
    def _purning(self, node):
        ...
        if not node.right.is_leaf():
            ...
            if res < minn:
                minn, minn_node = res, res_node
        subtree_num, subtree_loss = self.purning_loss(node)
        res = (node.gini * node.num_sample - subtree_loss) / (subtree_num - 1)
        if res < minn:
            minn, minn_node = res, node
        return minn, minn_node

    def purning_loss(self, node):
        if node.is_leaf():
            return 1, node.gini * node.num_sample
        else:
            lnum, lloss = self.purning_loss(node.left)
            rnum, rloss = self.purning_loss(node.right)
...
if __name__ == "__main__":
    ...
    clf = DTClassifier(max_depth=10, min_samples_leaf=2,alpha=.25,)
